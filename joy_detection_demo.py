...
class Animator(object):
    ...
    def __init__(self, leds, done):
        ...
        self._face_renderer = FaceRenderer()
    ...
    def update_joy_score(self, value):
        ...
        self._face_renderer.render(value)

    def join(self):
        self._thread.join()

class FaceRenderer(object):

    def __init__(self):
        self._face_to_load = ''
        self._throttle_time = 0
        self._display = Adafruit_SSD1306.SSD1306_128_64(rst=24, i2c_address=0x3C)
        self._display.begin()

    def render(self, value):
        if self._throttle_time > 2:
            if value > 0:
                if value >= 0.3:
                    face = str( int( round(value, 1) * 10 + 2 ) )
                else:
                    face = '2'
                
                if face != self._face_to_load:
                    image = Image.open( os.path.dirname(os.path.abspath(__file__)) + '/faces/face{0}.ppm'.format( face ) ).convert('1')
                    self._face_to_load = face
